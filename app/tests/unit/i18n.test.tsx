...
describe('i18n function tests', () => {
  describe('getFromLangLocaStorage()', () => {
    let langStorage: Record<string, string> = {}

    beforeAll(() => {
      global.Storage.prototype.setItem = vi.fn((key: string, value: string) => {
        langStorage[key] = value
      })
      global.Storage.prototype.getItem = vi.fn((key) => langStorage[key])
    })
  ...
  describe('getLangFromUrl()', () => {
    ...
    test('given an URL with a language, should return the lang from the URL', () => {
      const fullURL = new URL('/cat', baseURL)
      const language = getLangFromUrl(fullURL)

      expect(language).toBe('cat')
    })
